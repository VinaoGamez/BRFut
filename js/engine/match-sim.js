...
export function createRoundMatchSimulator(deps) {
  ...
  const simulateRoundMatch=(homeClub,awayClub,fixture=null)=>{
    ...
    const simulateMinute=(minute)=>{
      ...
      const hp=roundProfile(states.home,true),ap=roundProfile(states.away,false),dynamicTarget=clamp(50+(hp.passing-ap.passing)*.52+(hp.overall-ap.overall)*.30+(states.home.tactic.possession-states.away.tactic.possession)*.16+(states.home.tactic.press-states.away.tactic.press)*.035+(states.home.tactic.mentality-states.away.tactic.mentality)*.025+(states.home.momentum-states.away.momentum)*.3+3,29,71);homePossession=homePossession*.7+dynamicTarget*.3;metrics.home.possession=homePossession;metrics.away.possession=100-homePossession;
      ...
      const gap=attack.overall-defend.overall;const lead=side==='home'?homeGoals-awayGoals:awayGoals-homeGoals;const dom=engineDominanceModifiers({gap,lead,ownShots:m.shots,rivalShots:om.shots,minute});const creation=clamp((engineTuning.creationBase+(creator.passing*.46+creator.playmaking*.34+attacker.dribble*.12+attacker.speed*.08-defender.marking*.45-defender.tackling*.45)/130+(attack.passing-defend.defense)/130+(state.momentum-rival.momentum)/100)*dom.creationBoost,.22,.92);
