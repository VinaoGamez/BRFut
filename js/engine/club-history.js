...
function archiveSeason(archive, club) {
  ...
  [['Copa do Brasil', archive.cupCompetition], ['Recopa Nacional', archive.recopaCompetition], ['Campeonato Estadual', archive.stateLeagueSnapshot || archive.stateLeagueResults]].forEach(([label, source]) => { const games = fixtures(source).filter(game => game.home === club || game.away === club); if (!games.length) return; stats.competitions.push(label); games.forEach(game => addGame(stats, game, club)); });
...
export function buildClubHistory({ clubName, currentSeason, currentLogs = [], currentStanding = null, archives = [], rankingTitles = [] } = {}) {
  ...
  currentLogs.filter(log => log?.season == null || Number(log.season) === Number(currentSeason)).forEach(log => { addGame(current, log, clubName); if (log.competition) current.competitions.push(String(log.competition).replaceAll('_', ' ')); });
