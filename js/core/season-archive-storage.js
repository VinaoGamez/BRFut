...
export function commitSeasonArchiveFromLive(deps, meta = {}) {
  const archive = buildSeasonArchive({
    ...
    recopaCompetition: deps.getRecopaCompetition?.(),
    recopaFixtures: deps.getRecopaFixtures?.() || [],
    worldCupCompetition: deps.getWorldCupCompetition?.(),
    worldCupFixtures: deps.getWorldCupFixtures?.() || [],
    worldCupChampion: deps.getWorldCupChampion?.() || null,
    stateLeagueResults: deps.getStateLeagueResults?.() || {},
